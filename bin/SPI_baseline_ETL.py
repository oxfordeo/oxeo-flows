...
def reduce_SPI(aoi: Feature, R_seas: XRReducer, R_chirps: XRReducer) -> Feature:
    ...
    red_seas = R_seas.reduce(geom, "1981-01-01", "2019-12-31")
    red_chirps = R_chirps.reduce(geom, "1981-01-01", "2019-12-31")

    seas_df = red_seas.to_dataframe("tp")
    chirps_df = red_chirps.to_dataframe("tp")
    ...
    aoi["properties"]["seas2chirps"] = (
        chirps_df["tp"].mean() / seas_df_mean["tp"].mean()
    )
